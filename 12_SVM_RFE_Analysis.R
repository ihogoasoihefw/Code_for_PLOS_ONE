# ---------------------- 修改后的 SVM-RFE 算法 ----------------------
# 本代码经过修改，增加了一些冗余判断、进度条、步骤提示及详细注释
...
# setwd("D:\\文件1-25和文件38，网络毒理+分子模拟\\19.SVM-RFE")                 # 修改工作目录
...
PlotAccuracy <- function(accuracy, accuracy2 = NULL, no.info = 0.5,
                         ...
                         xlab = 'Number of Features', ylab = '0 x CV Accuracy') {
  oldPar <- par(mar = c(5, 5, 4, 5), xpd = TRUE)    # 保存原始图形参数
...
progressBar <- txtProgressBar(min = 1, max = 13, style = 3)  # 初始化进度条
featSweep <- lapply(1:13, function(i) {
