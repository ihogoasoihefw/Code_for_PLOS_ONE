# ========================== 0. 设置工作目录（可根据实际情况修改） ==========================
# setwd("D:\\文件1-25和文件38，网络毒理+分子模拟\\15.差异分析")  # 
...
# 关键修复：移除方差为0的基因（避免PCA标准化错误）
# 计算每个基因的方差（按行计算）
gene_variances <- apply(combined_expr, 1, var)
# 筛选出方差>0的基因（排除恒定值/全零基因）
combined_expr_filtered <- combined_expr[gene_variances > 0, ]
# 提示过滤信息
cat("PCA分析前过滤：共移除", sum(gene_variances == 0), "个方差为0的基因\n")

# 计算 PCA（使用过滤后的表达矩阵）
pca_result <- prcomp(t(combined_expr_filtered), scale. = TRUE)

# 构建 PCA 数据框
pca_df <- data.frame(
  Sample = colnames(combined_expr_filtered),  # 使用过滤后的列名（样本名）
  PC1 = pca_result$x[, 1],
  PC2 = pca_result$x[, 2],
