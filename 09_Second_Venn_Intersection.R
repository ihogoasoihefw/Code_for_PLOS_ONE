...
# 1. 设置工作目录
# setwd("D:\\文件1-25和文件38，网络毒理+分子模拟\\16.差异基因和药物分子基因取交集")

# 2. 创建输出文件夹
output_folder <- "output_folder"
if (!dir.exists(output_folder)) dir.create(output_folder)

# 3. 获取所有TXT和CSV文件并存储在列表中
files <- list.files(pattern = "\\.txt$|\\.csv$", full.names = TRUE)
...
for (file in files) {
  ...
  if (grepl("\\.txt$", file)) {
    ...
    rt <- read.csv(file, header = FALSE, check.names = FALSE)
  }
