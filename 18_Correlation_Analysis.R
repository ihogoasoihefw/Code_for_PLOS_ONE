...
library(pheatmap)
# 设置工作目录（请根据实际情况修改）
# setwd("D:\\文件1-25和文件38，网络毒理+分子模拟\\25.基因和免疫浸润评分的相关性分析")  # 设置当前工作目录
...
targetGeneFile <- "IntersectionGenes.csv"       # 待分析基因列表文件
...
# 保存图形风格A到PDF文件
pdf(file = "cor_newStyle.pdf", width = 12, height = 7)  # 打开PDF设备
print(plotA)            # 打印图形到设备
dev.off()               # 关闭PDF设备
...
# 计算目标基因与免疫细胞的相关性矩阵
expr <- exprMat  # 基因表达矩阵
immune <- immuneMat  # 免疫数据矩阵

# 初始化相关性矩阵
cor_mat <- matrix(NA, nrow = ncol(expr), ncol = ncol(immune))
rownames(cor_mat) <- colnames(expr)  # 设置基因为行名
colnames(cor_mat) <- colnames(immune)  # 设置免疫细胞为列名

# 计算每个基因与每个免疫细胞的Spearman相关性
for (i in seq_len(ncol(expr))) {
  for (j in seq_len(ncol(immune))) {
    cor_mat[i, j] <- cor(expr[, i], immune[, j], method = "spearman", use = "pairwise.complete.obs")
  }
}

# 计算P值并添加P值标注
p_val_matrix <- matrix(NA, nrow = ncol(expr), ncol = ncol(immune))
for (i in seq_len(ncol(expr))) {
  for (j in seq_len(ncol(immune))) {
    test_result <- cor.test(expr[, i], immune[, j], method = "spearman")
    p_val_matrix[i, j] <- test_result$p.value
  }
}

# 创建一个矩阵，用于显示相关性值和P值标注（根据P值显示1、2或3颗星）
labels_mat <- matrix("", nrow = nrow(cor_mat), ncol = ncol(cor_mat))
for (i in 1:nrow(cor_mat)) {
  for (j in 1:ncol(cor_mat)) {
    # 添加相关性值
    labels_mat[i, j] <- paste0(round(cor_mat[i, j], 2), "\n")
    # 根据P值添加星号
    if (p_val_matrix[i, j] < 0.001) {
      labels_mat[i, j] <- paste0(labels_mat[i, j], "***")
    } else if (p_val_matrix[i, j] < 0.01) {
      labels_mat[i, j] <- paste0(labels_mat[i, j], "**")
    } else if (p_val_matrix[i, j] < 0.05) {
      labels_mat[i, j] <- paste0(labels_mat[i, j], "*")
    }
  }
}
...
pdf_width <- 3 + 0.4 * immune_num  # 设定宽度
pdf_height <- 2 + 0.4 * gene_num  # 设定高度

# 绘制热图
pdf("Gene_Immune_Correlation_Heatmap.pdf", width = pdf_width, height = pdf_height)
pheatmap(
  cor_mat,  # 相关性矩阵
  color = my_col_fun,  # 使用渐变颜色
  display_numbers = labels_mat,  # 在每个单元格中显示相关性值和P值
  number_color = "black",  # 设置数字颜色为黑色
  cluster_rows = FALSE,  # 不进行行聚类
  cluster_cols = FALSE,  # 不进行列聚类
  fontsize_number = 9,  # 设置数字字体大小
  fontsize_row = 12,  # 设置行标签字体大小
  fontsize_col = 11,  # 设置列标签字体大小
  border_color = NA,  # 去除单元格边框
  main = "Gene-immune Correlation Heatmap"  # 设置主标题
)
dev.off()  # 关闭PDF设备


# 1. 读取免疫细胞数据文件（假设文件路径正确）
immune_data_path <- "CIBERSORT-Results.csv"  # 免疫细胞数据文件路径
immune_data <- read.table(immune_data_path, header = TRUE, sep = ",", check.names = FALSE, row.names = 1)  # 读取数据

# 确保免疫细胞数据被正确读取
cat("免疫细胞数据示例：\n")
head(immune_data)

# 2. 计算免疫细胞之间的 Pearson 相关性
cor_matrix <- cor(immune_data, method = "pearson")  # 计算Pearson相关性

# 3. 计算P值矩阵
p_val_matrix <- matrix(NA, nrow = ncol(immune_data), ncol = ncol(immune_data))  # 初始化P值矩阵

# 计算相关性和P值
for (i in 1:ncol(immune_data)) {
  for (j in 1:ncol(immune_data)) {
    test_result <- cor.test(immune_data[, i], immune_data[, j], method = "pearson")
    cor_matrix[i, j] <- test_result$estimate  # 存储相关系数
    p_val_matrix[i, j] <- test_result$p.value  # 存储P值
  }
}

# 4. 创建标签矩阵，显示相关性和P值的星号
labels_matrix <- matrix("", nrow = nrow(cor_matrix), ncol = ncol(cor_matrix))  # 初始化标签矩阵

for (i in 1:nrow(cor_matrix)) {
  for (j in 1:ncol(cor_matrix)) {
    if (!is.na(cor_matrix[i, j]) && !is.na(p_val_matrix[i, j])) {
      labels_matrix[i, j] <- paste0(round(cor_matrix[i, j], 2), "\n")  # 添加相关性值
      # 根据P值添加星号
      if (p_val_matrix[i, j] < 0.001) {
        labels_matrix[i, j] <- paste0(labels_matrix[i, j], "***")  # P值 < 0.001，显示 ***
      } else if (p_val_matrix[i, j] < 0.01) {
        labels_matrix[i, j] <- paste0(labels_matrix[i, j], "**")   # P值 < 0.01，显示 **
      } else if (p_val_matrix[i, j] < 0.05) {
        labels_matrix[i, j] <- paste0(labels_matrix[i, j], "*")    # P值 < 0.05，显示 *
      }
    }
  }
}
...
# 5. 定义颜色渐变
my_col_fun <- colorRampPalette(c("skyblue", "white", "#ba6262"))(100)  # 从绿色到红色的渐变色

# 6. 绘制热图并保存到PDF
pdf("immune_cells_correlation_heatmap_with_pvalues.pdf", width = 12, height = 12)  # 输出到PDF文件
pheatmap(cor_matrix,                          # 相关性矩阵
         color = my_col_fun,                   # 使用渐变颜色
         display_numbers = labels_matrix,       # 显示相关性值和显著性标记
         number_color = "black",               # 设置数字颜色为黑色
         fontsize_number = 8,                  # 设置数字字体大小
         main = "Immune Cells Correlation Heatmap with P-values",  # 设置标题
         cluster_rows = FALSE,                 # 不进行行聚类
         cluster_cols = FALSE,                 # 不进行列聚类
         fontsize = 12,                        # 设置字体大小
         labels_row = rownames(cor_matrix),     # 免疫细胞的行标签
         labels_col = colnames(cor_matrix)      # 免疫细胞的列标签
)
dev.off()  # 关闭PDF设备

cat("免疫细胞相关性热图（包含P值和标签）已保存为: immune_cells_correlation_heatmap_with_pvalues.pdf\n")  # 输出提示


cat("Gene-Immune Correlation Heatmap 已保存！\n")
